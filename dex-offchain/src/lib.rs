//!
//! DEX Offchain Library Entry
//!
//! 本模块为 DEX 离线服务库主入口，统一导出所有子模块，便于外部集成与调用。

// 导入并公开所有子模块。
pub mod config;    // 配置模块
pub mod factory;   // 工厂模块
pub mod jupiter;   // Jupiter 适配器模块
...
/// DEX 离线服务库版本号。
pub const DEX_OFFCHAIN_VERSION: &str = "1.0.0";
...
pub async fn main() -> std::io::Result<()> {
    ...
    // 配置热加载与Adapter自动初始化
    let config_path = "dex_config.toml";
    tokio::spawn(async move {
        config::watch_config(config_path, |cfg| {
            tracing::info!("[Config] DEX config changed: {:?}", cfg);
