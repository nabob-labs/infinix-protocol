...
pub fn handler<'info>(
    ...
    let infinix = ctx.accounts.infinix.to_account_info();
    let infinix_data = ctx.accounts.infinix.load()?;
    ...
    for raw_amount in raw_amounts {
        ...
        let signer_seeds = &[
            INFINIX_SEEDS,
            infinix_data.infinix_token_mint.as_ref(),
            &[infinix_data.bump],
        ];
    ...
    user_pending_basket.remove_token_amounts_from_infinix(
        &removed_mints,
        true,
