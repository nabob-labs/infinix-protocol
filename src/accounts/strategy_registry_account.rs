...
use crate::strategies::strategy_registry::StrategyConfig; // 引入策略配置结构体，支持灵活扩展
...
impl StrategyRegistryAccount {
    ...
    /// - 参数 config: 策略配置结构体
    /// - 参数 creator: 策略创建者公钥
    /// - 返回 Result<()>，成功则策略被添加到注册表
    pub fn register_strategy(&mut self, id: u64, config: StrategyConfig, creator: Pubkey) -> Result<()> {
    ...
    /// - 参数 to: 需激活的策略 ID
    /// - 返回 Result<()>，成功则切换激活状态
    pub fn switch_strategy(&mut self, from: u64, to: u64) -> Result<()> {
