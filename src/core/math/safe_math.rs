...
impl SafeMath {
    ...
    /// 精确除法（带小数）
    pub fn div_precise(a: u64, b: u64, precision: u8) -> Result<u64> {
        if b == 0 {
            return Err(StrategyError::MathOverflow);
        }
    ...
    /// 百分比基点计算
    pub fn percentage_bps(amount: u64, percentage_bps: u64) -> Result<u64> {
        amount.checked_mul(percentage_bps).ok_or(StrategyError::MathOverflow)?.checked_div(10_000).ok_or(StrategyError::MathOverflow)
    }
    /// 复利计算
    pub fn compound_interest(principal: u64, rate_bps: u64, periods: u32) -> Result<u64> {
