...
//!
//! 本模块实现 Meteora DEX 适配器，集成 Anchor CPI 调用，支持流动性管理、报价、异常处理等，确保链上集成合规、可维护。
...
/// 实现 DexAdapter trait，集成 Meteora 链上 CPI 调用。
impl DexAdapter for MeteoraAdapter {
    ...
    fn swap(&self, ctx: Context<Swap>, params: SwapParams) -> Result<SwapResult> {
        ...
        // 构造 CPI 所需账户。
        let cpi_accounts = meteora::cpi::accounts::Swap {
        ...
        // 调用 Meteora CPI swap。
        let result = meteora::cpi::swap(cpi_ctx, params.amount_in, params.min_amount_out)?;
        // 返回 swap 结果。
        Ok(SwapResult {
    ...
    fn add_liquidity(&self, ctx: Context<AddLiquidity>, params: AddLiquidityParams) -> Result<u64> {
        let cpi_accounts = meteora::cpi::accounts::AddLiquidity {
            user: ctx.accounts.user.to_account_info(),
            pool: ctx.accounts.pool.to_account_info(),
        ...
        let cpi_ctx = CpiContext::new(ctx.accounts.meteora_program.to_account_info(), cpi_accounts);
        let result = meteora::cpi::add_liquidity(cpi_ctx, params.amount_a, params.amount_b)?;
        Ok(result.liquidity)
    }
    /// 移除流动性。
    fn remove_liquidity(&self, ctx: Context<RemoveLiquidity>, params: RemoveLiquidityParams) -> Result<u64> {
        let cpi_accounts = meteora::cpi::accounts::RemoveLiquidity {
            user: ctx.accounts.user.to_account_info(),
            pool: ctx.accounts.pool.to_account_info(),
        ...
        let cpi_ctx = CpiContext::new(ctx.accounts.meteora_program.to_account_info(), cpi_accounts);
        let result = meteora::cpi::remove_liquidity(cpi_ctx, params.liquidity)?;
        Ok(result.amount_out)
    }
    /// 获取报价。
    fn get_quote(&self, ctx: Context<GetQuote>, params: QuoteParams) -> Result<QuoteResult> {
        let cpi_accounts = meteora::cpi::accounts::GetQuote {
            pool: ctx.accounts.pool.to_account_info(),
            input_vault: ctx.accounts.input_vault.to_account_info(),
            output_vault: ctx.accounts.output_vault.to_account_info(),
        };
        let cpi_ctx = CpiContext::new(ctx.accounts.meteora_program.to_account_info(), cpi_accounts);
        let quote = meteora::cpi::get_quote(cpi_ctx, params.input_mint, params.output_mint, params.amount_in)?;
        Ok(QuoteResult {
