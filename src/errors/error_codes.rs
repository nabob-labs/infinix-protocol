...
pub fn validate_error_code_range(error_code: u32) -> bool {
    const ASSET_ERROR_END: u32 = ASSET_ERROR_BASE + 999;
    const BASKET_ERROR_END: u32 = BASKET_ERROR_BASE + 999;
    const INDEX_TOKEN_ERROR_END: u32 = INDEX_TOKEN_ERROR_BASE + 999;
    const DEX_ERROR_END: u32 = DEX_ERROR_BASE + 999;
    const ORACLE_ERROR_END: u32 = ORACLE_ERROR_BASE + 999;
    const ALGORITHM_ERROR_END: u32 = ALGORITHM_ERROR_BASE + 999;
    const STRATEGY_ERROR_END: u32 = STRATEGY_ERROR_BASE + 999;
    const VALIDATION_ERROR_END: u32 = VALIDATION_ERROR_BASE + 999;
    const SECURITY_ERROR_END: u32 = SECURITY_ERROR_BASE + 999;
    
    matches!(
        error_code,
        ASSET_ERROR_BASE..=ASSET_ERROR_END |
        BASKET_ERROR_BASE..=BASKET_ERROR_END |
        INDEX_TOKEN_ERROR_BASE..=INDEX_TOKEN_ERROR_END |
        DEX_ERROR_BASE..=DEX_ERROR_END |
        ORACLE_ERROR_BASE..=ORACLE_ERROR_END |
        ALGORITHM_ERROR_BASE..=ALGORITHM_ERROR_END |
        STRATEGY_ERROR_BASE..=STRATEGY_ERROR_END |
        VALIDATION_ERROR_BASE..=VALIDATION_ERROR_END |
        SECURITY_ERROR_BASE..=SECURITY_ERROR_END
    )
}
...
pub fn get_error_module(error_code: u32) -> &'static str {
    ...
    match error_code {
        ASSET_ERROR_BASE..=ASSET_ERROR_END => "Asset",
        BASKET_ERROR_BASE..=BASKET_ERROR_END => "Basket",
        INDEX_TOKEN_ERROR_BASE..=INDEX_TOKEN_ERROR_END => "IndexToken",
        DEX_ERROR_BASE..=DEX_ERROR_END => "DEX",
        ORACLE_ERROR_BASE..=ORACLE_ERROR_END => "Oracle",
        ALGORITHM_ERROR_BASE..=ALGORITHM_ERROR_END => "Algorithm",
        STRATEGY_ERROR_BASE..=STRATEGY_ERROR_END => "Strategy",
        VALIDATION_ERROR_BASE..=VALIDATION_ERROR_END => "Validation",
        SECURITY_ERROR_BASE..=SECURITY_ERROR_END => "Security",
        _ => "Unknown",
    }
}
