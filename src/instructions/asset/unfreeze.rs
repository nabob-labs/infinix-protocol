//!
//! Asset Unfreeze Instruction
//! 资产解冻指令实现，所有业务逻辑下沉到 service 层，指令层只做参数校验、账户校验、事件触发。
...
/// - 返回: Anchor规范Result
pub fn unfreeze_asset(ctx: Context<UnfreezeAsset>) -> anchor_lang::Result<()> {
    ...
    // 业务逻辑：解冻资产
    AssetService::unfreeze(asset, ctx.accounts.authority.key())?;
    emit!(AssetUnfrozen {
