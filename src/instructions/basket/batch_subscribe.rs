//!
//! Basket Batch Subscribe Instruction
//! 篮子批量认购指令最小功能单元实现，严格遵循Anchor规范、SOLID原则、分层设计、接口清晰、类型安全、事件追踪、权限校验、生命周期管理、错误处理、逐行注释，生产级代码质量。
...
#[derive(Accounts)]
pub struct BatchSubscribeBasket<'info> {
    #[account(mut)]
...
pub fn batch_subscribe_basket(
    ctx: Context<BatchSubscribeBasket>,
    amounts: Vec<u64>,
) -> anchor_lang::Result<()> {
    let basket = &mut ctx.accounts.basket;
    // 权限校验
    require_keys_eq!(basket.authority, ctx.accounts.authority.key(), crate::errors::basket_error::BasketError::NotAllowed);
    // 批量认购操作（实际业务逻辑应在服务层实现）
    BasketService::batch_subscribe(basket, &amounts)?;
    emit!(BasketBatchSubscribed {
        basket_id: basket.id,
