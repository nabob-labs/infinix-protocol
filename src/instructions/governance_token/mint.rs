//! Governance Token铸造指令模块
//! 
//! 本模块提供Governance Token资产的铸造功能，包括：
//! - 参数验证：验证铸造参数的有效性和边界条件
//! - 权限检查：验证铸造权限和授权状态
//! - 服务层调用：委托给GovernanceTokenService执行核心业务逻辑
//! - 事件发射：发射Governance Token铸造事件用于审计和追踪
//! 
//! 设计特点：
//! - 最小功能单元：专注于Governance Token铸造功能
//! - 类型安全：严格的类型检查和边界验证
//! - 参数验证：全面的输入参数验证
//! - 权限控制：细粒度的权限验证
...
/// Governance Token铸造参数结构体
/// 
/// 包含Governance Token铸造所需的所有参数：
/// - amount: 铸造数量
/// - governance_id: 治理ID
/// - exec_params: 执行参数
#[derive(AnchorSerialize, AnchorDeserialize, Clone, Debug, PartialEq)]
pub struct MintGovernanceParams {
    /// 铸造数量
    pub amount: u64,
    /// 治理ID
...
/// Governance Token铸造账户上下文
/// 
/// 定义Governance Token铸造指令所需的账户结构：
...
#[derive(Accounts)]
pub struct MintGovernance<'info> {
    ...
    #[account(
        constraint = authority.key() == governance_token.mint_authority @ AssetError::InvalidMintAuthority
    )]
...
/// 验证Governance Token铸造参数
/// 
/// 检查Governance Token铸造参数的有效性和边界条件：
/// - 铸造数量验证
/// - 治理ID验证
/// - 执行参数验证
/// 
/// # 参数
/// - params: Governance Token铸造参数
/// 
/// # 返回
/// - Result<()>: 验证结果
pub fn validate_mint_governance_params(params: &MintGovernanceParams) -> Result<()> {
    // 验证铸造数量
    require!(
        params.amount > 0,
...
/// 检查Governance Token铸造权限
/// 
/// 验证Governance Token铸造权限和授权状态：
/// - 检查铸造权限
/// - 验证授权状态
/// 
/// # 参数
...
pub fn check_mint_authority_permission(
    ...
    // 检查铸造权限
    require!(
        authority.key() == governance_token.mint_authority,
...
/// Governance Token铸造指令
/// 
/// 执行Governance Token铸造操作，包括：
/// - 参数验证：验证铸造参数的有效性
/// - 权限检查：验证铸造权限
/// - 服务层调用：委托给GovernanceTokenService执行铸造逻辑
/// - 事件发射：发射Governance Token铸造事件
/// 
/// # 参数
/// - ctx: Governance Token铸造账户上下文
/// - params: Governance Token铸造参数
/// 
/// # 返回
/// - Result<()>: 铸造操作结果
pub fn mint_governance_token(
    ctx: Context<MintGovernance>,
    params: MintGovernanceParams,
) -> Result<()> {
    // 参数验证
    validate_mint_governance_params(&params)?;
    
    // 权限检查
    check_mint_authority_permission(&ctx.accounts.authority, &ctx.accounts.governance_token)?;
