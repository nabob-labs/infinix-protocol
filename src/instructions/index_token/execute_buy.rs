//!
//! execute_buy.rs - 指数代币执行买入指令
//!
//! 本文件实现指数代币执行买入指令，严格遵循Rust、Solana、Anchor、SOLID最佳实践，
//! 并逐行专业注释，便于审计、维护、扩展。
...
pub fn execute_buy_index_token(
    ...
    // 校验买方签名
    require!(ctx.accounts.buyer.is_signer, crate::errors::index_token_error::IndexTokenError::NotAllowed);
    // 实际业务应调用服务层或核心买入逻辑，这里仅做事件示例
    emit!(IndexTokenBuyExecuted {
