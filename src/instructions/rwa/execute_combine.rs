...
/// RWA资产执行合并指令实现
pub fn execute_combine_rwa(ctx: Context<ExecuteCombineRwa>, amount: u64, params: TradeParams) -> Result<()> {
    let target = &mut ctx.accounts.target;
    let source = &mut ctx.accounts.source;
    target.validate()?;
    source.validate()?;
    require!(target.asset_type == AssetType::RWA, crate::error::ProgramError::InvalidAssetType);
