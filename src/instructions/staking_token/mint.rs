//! Staking Token铸造指令模块
//! 
//! 本模块提供Staking Token资产的铸造功能，包括：
//! - 参数验证：验证铸造参数的有效性和边界条件
//! - 权限检查：验证铸造权限和授权状态
//! - 服务层调用：委托给StakingTokenService执行核心业务逻辑
//! - 事件发射：发射Staking Token铸造事件用于审计和追踪
//! 
//! 设计特点：
//! - 最小功能单元：专注于Staking Token铸造功能
//! - 类型安全：严格的类型检查和边界验证
//! - 参数验证：全面的输入参数验证
//! - 权限控制：细粒度的权限验证
...
/// Staking Token铸造参数结构体
/// 
/// 包含Staking Token铸造所需的所有参数：
/// - amount: 铸造数量
/// - staking_pool_id: 质押池ID
/// - exec_params: 执行参数
#[derive(AnchorSerialize, AnchorDeserialize, Clone, Debug, PartialEq)]
pub struct MintStakingParams {
    /// 铸造数量
    pub amount: u64,
    /// 质押池ID
...
/// Staking Token铸造账户上下文
/// 
/// 定义Staking Token铸造指令所需的账户结构：
...
pub struct MintStaking<'info> {
    ...
    #[account(
        constraint = authority.key() == staking_token.mint_authority @ AssetError::InvalidMintAuthority
    )]
...
/// 验证Staking Token铸造参数
/// 
/// 检查Staking Token铸造参数的有效性和边界条件：
/// - 铸造数量验证
/// - 质押池ID验证
/// - 执行参数验证
/// 
/// # 参数
/// - params: Staking Token铸造参数
/// 
/// # 返回
/// - Result<()>: 验证结果
pub fn validate_mint_staking_params(params: &MintStakingParams) -> Result<()> {
    // 验证铸造数量
    require!(
        params.amount > 0,
...
pub fn check_mint_authority_permission(
    ...
    // 检查铸造权限
    require!(
        authority.key() == staking_token.mint_authority,
...
/// Staking Token铸造指令
/// 
/// 执行Staking Token铸造操作，包括：
/// - 参数验证：验证铸造参数的有效性
/// - 权限检查：验证铸造权限
/// - 服务层调用：委托给StakingTokenService执行铸造逻辑
/// - 事件发射：发射Staking Token铸造事件
/// 
/// # 参数
/// - ctx: Staking Token铸造账户上下文
/// - params: Staking Token铸造参数
/// 
/// # 返回
/// - Result<()>: 铸造操作结果
pub fn mint_staking_token(
    ctx: Context<MintStaking>,
    params: MintStakingParams,
) -> Result<()> {
    // 参数验证
    validate_mint_staking_params(&params)?;
    
    // 权限检查
    check_mint_authority_permission(&ctx.accounts.authority, &ctx.accounts.staking_token)?;
