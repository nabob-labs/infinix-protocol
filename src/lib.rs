...
use crate::core::registry::{DEX_ADAPTER_REGISTRY, ORACLE_ADAPTER_REGISTRY}; // 自动注册DEX/Oracle适配器，提升可扩展性

// === 模块声明 ===
/// 业务域模块分层声明，便于维护与扩展，每个模块均为独立功能域
pub mod accounts;    ///< 账户模型与账户校验，定义所有链上账户结构和校验逻辑
...
pub mod error;       ///< 错误类型与处理，定义所有错误码与处理逻辑
...
pub mod index_token_program { // 定义主合约模块，所有链上指令均在此注册
    ...
    pub fn mint_asset(ctx: Context<instructions::asset::MintAsset>, amount: u64) -> Result<()> { // 资产增发指令主函数，ctx为账户上下文，amount为增发数量
        // 1. Anchor自动生成的账户上下文，包含所有所需账户及权限校验
        // 2. amount参数为增发数量，类型安全，需业务层校验正整数
        // 3. 调用 asset 模块的 mint_asset 实现，完成实际增发逻辑
        // 4. 返回下层实现的结果，Result<()>类型，Anchor自动处理错误与生命周期
    ...
    /// - Anchor自动校验账户权限、生命周期、PDA
    /// - 需确保amount参数合法，防止溢出与非法操作
    pub fn burn_asset(ctx: Context<instructions::asset::BurnAsset>, amount: u64) -> Result<()> { // 资产销毁指令主函数，ctx为账户上下文，amount为销毁数量
        // 1. Anchor自动生成的账户上下文，包含所有所需账户及权限校验
    ...
    /// - Anchor自动校验账户权限、生命周期、PDA
    /// - 需确保amount参数合法，防止溢出与非法操作
    pub fn transfer_asset(ctx: Context<instructions::asset::TransferAsset>, amount: u64) -> Result<()> { // 资产转账指令主函数，ctx为账户上下文，amount为转账数量
        // 1. Anchor自动生成的账户上下文，包含所有所需账户及权限校验
    ...
    /// # 安全性
    /// - Anchor自动校验账户权限、生命周期、PDA
    pub fn query_asset(ctx: Context<instructions::asset::QueryAsset>) -> Result<u64> { // 资产查询指令主函数，ctx为账户上下文
        // 1. Anchor自动生成的账户上下文，包含所有所需账户及权限校验
    ...
    pub fn combine_asset(
        ...
        // 5. 返回下层实现的结果，Result<()>类型，Anchor自动处理错误与生命周期
    ...
    ) -> Result<u64> { // Anchor标准返回类型，返回资产报价
        // 1. Anchor自动生成的账户上下文，包含所有所需账户及权限校验
    ...
    pub fn batch_strategy_trade_asset(
        ...
        // 1. Anchor自动生成的账户上下文，包含所有所需账户及权限校验
        // 2. strategies为类型安全参数，需业务层校验
        // 3. exec_params为可选类型安全参数
        // 4. 调用 asset 模块的 batch_strategy_trade_asset 实现，完成实际批量策略交易逻辑
    ...
    pub fn rebalance_basket_with_algo(
        ...
        params: crate::algorithms::traits::ExecutionParams, // 算法执行参数，类型安全，需业务层校验
    ) -> Result<()> { // Anchor标准返回类型，表示指令执行成功或失败
        // 1. Anchor自动生成的账户上下文，包含所有所需账户及权限校验
    ...
    ) -> Result<u64> { // Anchor标准返回类型，返回篮子余额
        // 1. Anchor自动生成的账户上下文，包含所有所需账户及权限校验
    ...
    pub fn authorize_basket(
        ctx: Context<instructions::basket::AuthorizeBasket>, // Anchor账户上下文，自动校验账户权限与生命周期
        exec_params: Option<AlgoParams>, // 可选算法参数，类型安全，需业务层校验
        strategy_params: Option<StrategyParams>, // 可选策略参数，类型安全，需业务层校验
    ) -> Result<()> { // Anchor标准返回类型，表示指令执行成功或失败
        // Anchor自动生成账户上下文，参数类型安全，业务层校验，调用 basket 模块实现
        instructions::basket::authorize_basket(ctx, exec_params, strategy_params) // 调用实际授权实现，返回执行结果
    }
    ...
    ) -> Result<u64> { // Anchor标准返回类型，返回篮子报价
    ...
    ) -> Result<u64> { // Anchor标准返回类型，返回指数代币余额
    ...
    pub fn quote_index_token(
        ...
        params: TradeParams, // 交易参数，类型安全，需业务层校验
        price_params: OracleParams, // 价格参数，类型安全，需业务层校验
    ) -> Result<u64> { // Anchor标准返回类型，返回指数代币报价
