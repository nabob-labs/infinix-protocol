...
impl EtfService {
    ...
    pub fn mint(&self, basket: &mut BasketIndexState, amount: u64) -> anchor_lang::Result<()> {
        // 校验资产类型
        require!(basket.asset_type == crate::core::types::AssetType::ETF, solana_program_error::ProgramError::InvalidAssetType);
        // 校验账户激活状态
    ...
    pub fn transfer(&self, from: &mut BasketIndexState, to: &mut BasketIndexState, amount: u64) -> anchor_lang::Result<()> {
        // 校验资产类型
        require!(from.asset_type == crate::core::types::AssetType::ETF, solana_program_error::ProgramError::InvalidAssetType);
        require!(to.asset_type == crate::core::types::AssetType::ETF, solana_program_error::ProgramError::InvalidAssetType);
        // 校验账户激活状态
    ...
    /// - basket: ETF资产账户
    /// 返回: 资产信息结构体
    pub fn query(&self, basket: &BasketIndexState) -> anchor_lang::Result<EtfInfo> {
        // 校验资产类型
        require!(basket.asset_type == crate::core::types::AssetType::ETF, solana_program_error::ProgramError::InvalidAssetType);
        Ok(EtfInfo {
    ...
    /// - amount: 询价数量
    /// 返回: 报价信息结构体
    pub fn quote(&self, basket: &BasketIndexState, amount: u64) -> anchor_lang::Result<EtfQuote> {
        // 校验资产类型
        require!(basket.asset_type == crate::core::types::AssetType::ETF, solana_program_error::ProgramError::InvalidAssetType);
        // 校验数量
    ...
    /// - basket: ETF资产账户，需可变
    /// - actions: 批量操作类型与参数数组
    pub fn batch(&self, basket: &mut BasketIndexState, actions: &[EtfBatchAction]) -> anchor_lang::Result<()> {
        // 校验资产类型
        require!(basket.asset_type == crate::core::types::AssetType::ETF, solana_program_error::ProgramError::InvalidAssetType);
        // 遍历批量操作
