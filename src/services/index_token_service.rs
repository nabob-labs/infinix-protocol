...
impl IndexTokenService {
    ...
    pub fn mint(&self, basket: &mut BasketIndexState, amount: u64) -> Result<()> {
        // 校验资产类型
        require!(basket.asset_type == crate::core::types::AssetType::IndexToken, crate::error::ProgramError::InvalidAssetType);
        // 校验账户激活状态
    ...
    pub fn transfer(&self, from: &mut BasketIndexState, to: &mut BasketIndexState, amount: u64) -> Result<()> {
        // 校验资产类型
        require!(from.asset_type == crate::core::types::AssetType::IndexToken, crate::error::ProgramError::InvalidAssetType);
        require!(to.asset_type == crate::core::types::AssetType::IndexToken, crate::error::ProgramError::InvalidAssetType);
        // 校验账户激活状态
    ...
    /// 返回: 资产信息结构体
    pub fn query(&self, basket: &BasketIndexState) -> Result<IndexTokenInfo> {
        // 校验资产类型
        require!(basket.asset_type == crate::core::types::AssetType::IndexToken, crate::error::ProgramError::InvalidAssetType);
        Ok(IndexTokenInfo {
    ...
    /// - amount: 询价数量
    /// 返回: 报价信息结构体
    pub fn quote(&self, basket: &BasketIndexState, amount: u64) -> Result<IndexTokenQuote> {
        // 校验资产类型
        require!(basket.asset_type == crate::core::types::AssetType::IndexToken, crate::error::ProgramError::InvalidAssetType);
        // 校验数量
    ...
    /// - basket: IndexToken资产账户，需可变
    /// - actions: 批量操作类型与参数数组
    pub fn batch(&self, basket: &mut BasketIndexState, actions: &[IndexTokenBatchAction]) -> Result<()> {
        // 校验资产类型
        require!(basket.asset_type == crate::core::types::AssetType::IndexToken, crate::error::ProgramError::InvalidAssetType);
        // 遍历批量操作
