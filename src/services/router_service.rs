...
trait RoutingStrategyRegistrable {
    /// 注册路由策略
    ///
    /// # 参数
    /// - `strategy`: 策略实例。
    ...
    fn register_strategy(&self, strategy: Box<dyn RoutingStrategy>) -> anchor_lang::Result<()>;
}
...
impl RoutingStrategyRegistrable for RegisterRoutingStrategyService {
    /// 注册实现
    fn register_strategy(&self, _strategy: Box<dyn RoutingStrategy>) -> anchor_lang::Result<()> {
        // 生产级实现：注册到全局策略表
        Ok(())
    }
}
...
trait PathSelectable {
    ...
    fn select_path(&self, params: &RoutingParams) -> anchor_lang::Result<RoutingResult>;
}
...
trait BatchRoutable {
    ...
    fn batch_route(&self, batch_params: &BatchRoutingParams) -> anchor_lang::Result<Vec<RoutingResult>>;
}
