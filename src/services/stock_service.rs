//! StockService - Stock资产服务层
//! 生产级最小功能单元，SOLID原则、合规注释

use crate::state::baskets::BasketIndexState;
use anchor_lang::prelude::*;

/// StockService结构体，Stock资产服务实现
pub struct StockService;

impl StockService {
    /// 构造函数，返回StockService实例
    pub fn new() -> Self {
        Self
    }

    /// Stock资产mint最小功能单元
    /// - basket: Stock资产账户，需可变
    /// - amount: 增发数量，类型安全
    pub fn mint(&self, basket: &mut BasketIndexState, amount: u64) -> anchor_lang::Result<()> {
        // 校验资产类型
        require!(basket.asset_type == crate::core::types::AssetType::Stock, solana_program_error::ProgramError::InvalidAssetType);
        // 校验账户激活状态
    ...
    /// Stock资产transfer最小功能单元
    /// - from: 转出账户，需可变
    /// - to: 转入账户，需可变
    /// - amount: 转账数量，类型安全
    pub fn transfer(&self, from: &mut BasketIndexState, to: &mut BasketIndexState, amount: u64) -> anchor_lang::Result<()> {
        // 校验资产类型
        require!(from.asset_type == crate::core::types::AssetType::Stock, solana_program_error::ProgramError::InvalidAssetType);
        require!(to.asset_type == crate::core::types::AssetType::Stock, solana_program_error::ProgramError::InvalidAssetType);
        // 校验账户激活状态
    ...
    /// - basket: Stock资产账户
    /// 返回: 资产信息结构体
    pub fn query(&self, basket: &BasketIndexState) -> anchor_lang::Result<StockInfo> {
        // 校验资产类型
        require!(basket.asset_type == crate::core::types::AssetType::Stock, solana_program_error::ProgramError::InvalidAssetType);
        Ok(StockInfo {
    ...
    /// - amount: 询价数量
    /// 返回: 报价信息结构体
    pub fn quote(&self, basket: &BasketIndexState, amount: u64) -> anchor_lang::Result<StockQuote> {
        // 校验资产类型
        require!(basket.asset_type == crate::core::types::AssetType::Stock, solana_program_error::ProgramError::InvalidAssetType);
        // 校验数量
    ...
    /// - basket: Stock资产账户，需可变
    /// - actions: 批量操作类型与参数数组
    pub fn batch(&self, basket: &mut BasketIndexState, actions: &[StockBatchAction]) -> anchor_lang::Result<()> {
        // 校验资产类型
        require!(basket.asset_type == crate::core::types::AssetType::Stock, solana_program_error::ProgramError::InvalidAssetType);
        // 遍历批量操作
